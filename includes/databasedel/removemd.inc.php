...
// a munkadíköltségből töröl

if($id > 0){
  $checkRecord = mysqli_query($conn,"SELECT * FROM munkadijkoltseg WHERE munkadijkoltseg_id=".$id);
  ...
  if($totalrows > 0){
    ...
    $stmt = $conn->prepare("DELETE FROM munkadijkoltseg WHERE munkadijkoltseg_id = ?");
